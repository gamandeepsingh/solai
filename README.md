# SOLAI Wallet 🟢

![License](https://img.shields.io/badge/license-MIT-green) ![Version](https://img.shields.io/badge/version-1.0.2-blue) ![Platform](https://img.shields.io/badge/platform-Chrome%20Extension-yellow)

Minimal, AI-first Solana wallet built as a Chrome Extension.
<img width="1280" height="800" alt="1" src="https://github.com/user-attachments/assets/ff16e8e7-c422-4852-8b4b-40467f74e633" />

---

## ✨ Overview

SOLAI Wallet is a **simple and friendly crypto wallet** focused on making Solana easy to use.  
Clean UI, playful design, and future-ready AI interactions.

---

## 🚀 Features (MVP)

- Solana wallet (create / import)
- View balance (SOL + SPL tokens)
- Send / Receive tokens
- Save contacts
- Minimal AI input UI (coming soon)
- Light + Dark mode

---

## 🔐 Security

- Non-custodial wallet
- Private keys stored locally
- No backend key access

---

## 🧠 Vision

Turn crypto into **simple commands**:

> "Send $5 to mom every day"  
> "Swap USDC to SOL"
> "Send 1SOL to Joe"

---

## 📦 Install without building (Developer Mode)

The easiest way to try SOLAI — no Node.js or build tools needed.

**1. Download the extension**

- Go to the [Releases](../../releases) page on GitHub
- Download the latest `v1.0.2.zip`
- Unzip it anywhere on your computer — you'll get a folder called `dist/`

**2. Load into Chrome**

1. Open Chrome and go to `chrome://extensions`
2. Toggle **Developer mode** on (top-right corner)
3. Click **Load unpacked**
4. Select the unzipped `dist/` folder

SOLAI will appear in your extensions bar. Pin it for quick access.

> Works on Chrome, Brave, Arc, and any Chromium-based browser.

---

## ⚡ Getting Started (build from source)

```bash
pnpm install
pnpm build
```

Then load the built output:

1. Go to `chrome://extensions`
2. Enable **Developer mode**
3. Click **Load unpacked**
4. Select the `extension/dist/` folder

---

## 🛡️ Security

Found a vulnerability? Please **do not** open a public issue.
See [SECURITY.md](./SECURITY.md) for responsible disclosure instructions.

---

## 📄 License

Copyright (c) 2025 Gamandeep. Released under the [MIT License](./LICENSE).
