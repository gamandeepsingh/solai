...
export default function AIChatScreen() {
  const { messages, isStreaming, sendMessage, abort } = useAI()
  ...
  useEffect(() => {
    ...
    if (initial && hasKey) {
      sendSystemMessage(initial)
    }
  }, [hasKey])

  function buildContext() {
    const sol = balances.find(b => b.meta.symbol === 'SOL')?.amount ?? 0
    return `You are SOLAI, a friendly Solana wallet assistant. Wallet: ${account?.publicKey ?? 'unknown'}. SOL balance: ${sol.toFixed(4)} SOL. Network: ${network}. Be concise and friendly. Never ask for private keys or seed phrases.`
  }

  async function sendSystemMessage(content: string) {
    setError('')
    try {
      await sendMessage(content, buildContext())
    } catch (e: any) {
      setError(e.message?.includes('No API key') ? 'Add your OpenRouter API key in Settings' : 'Something went wrong')
    }
  }
    ...
    <div className="h-full flex flex-col bg-[var(--color-bg)]">
      <div className="flex items-center gap-3 px-4 pt-4 pb-2 border-b border-[var(--color-border)]">
        ...
        <div>
          ...
          <p className="text-[10px] opacity-40">{isStreaming ? 'Typing...' : 'Online'}</p>
        </div>
        {isStreaming && (
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 pb-0 flex flex-col gap-3">
        {messages.length === 0 && !hasKey && (
          <div className="flex flex-col items-center gap-3 mt-10 text-center">
            ...
            >Go to Settings</motion.button>
          </div>
        )}
        {messages.length === 0 && hasKey && (
          <div className="flex flex-col items-center gap-2 mt-10 text-center opacity-40">
            <span className="text-3xl">✨</span>
            <p className="text-sm">Ask me anything about Solana</p>
          </div>
        )}
        <AnimatePresence>
          {messages.map(msg => (
